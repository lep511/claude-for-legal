...
1. **Read the allowlist first.** `claude-for-legal/agents/legal-builder-hub/allowlist.yaml`. If restrictive mode and source not listed: refuse. If permissive: warn and continue.
...
7. **Install.** Copy the directory. Update `claude-for-legal/agents/legal-builder-hub/CLAUDE.md` and append to `install-log.yaml`.
...
- Instructions to read files outside `claude-for-legal/plugins/config/` or the skill's
  own directory
- Instructions to write files outside the skill's own directory — especially
  to `claude-for-legal/`, any `CLAUDE.md`, `.gitignore`, shell configs, or launchd
  paths
...
- **File-write paths** — does any instruction write to `claude-for-legal/`, any
...
- If it's standalone: `claude-for-legal/skills/[skill-name]/`
...
Record in `claude-for-legal/agents/legal-builder-hub/CLAUDE.md`
