...
The user typed the customize workflow. They want to change something in their
...
1. **Read the config.** Read
   ...
   (and `claude-for-legal/agents/company-profile.md` one
   level up). If the plugin config does not exist or still contains
   `[PLACEHOLDER]` values, say:

   > You haven't run setup yet. Configure this agent in Settings first —
...
5. **For shared-profile changes** (company name, industry, jurisdictions,
   ...
   `claude-for-legal/agents/company-profile.md` and note:
