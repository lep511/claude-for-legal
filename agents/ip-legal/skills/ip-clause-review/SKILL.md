...
Reviews the IP clauses in an agreement against the practice profile in `claude-for-legal/agents/ip-legal/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.
...
**Escalation:** Per `claude-for-legal/agents/ip-legal/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `claude-for-legal/agents/ip-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
