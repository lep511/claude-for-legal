...
2. Load `claude-for-legal/agents/law-student/CLAUDE.md` → jurisdiction, exam format, weak subjects.
3. Load `claude-for-legal/agents/law-student/study-plan.yaml` if it exists. Read `session_history` for this subject to weight subtopics toward where the student has been weak.
...
6. At session end, write session results:
   ...
   - If not: write to `claude-for-legal/agents/law-student/session-history.yaml`.
7. Report:
