...
1. Check `claude-for-legal/agents/legal-clinic/CLAUDE.md`. If populated and no `--redo`, confirm before overwriting.
...
Read `claude-for-legal/agents/legal-clinic/CLAUDE.md`:
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated CLAUDE.md exists at the old cache path `claude-for-legal/plugins/cache/claude-for-legal/legal-clinic/*/CLAUDE.md` but not here, copy it forward first.
