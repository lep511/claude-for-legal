...
1. Load `claude-for-legal/agents/legal-clinic/CLAUDE.md` → practice-area templates, jurisdiction, local rules, supervision style.
...
`claude-for-legal/agents/legal-clinic/CLAUDE.md` → practice areas, practice-area templates, jurisdiction (state + local court + any local rules ingested), supervision style.
...
Match the request to the clinic's template set (from `claude-for-legal/agents/legal-clinic/CLAUDE.md`). Common set by practice area:
...
Per `claude-for-legal/agents/legal-clinic/CLAUDE.md` jurisdiction:
...
Filing a document with a court or agency is a consequential action. The gate is the supervision workflow in `## Supervision style` in `claude-for-legal/agents/legal-clinic/CLAUDE.md`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. Court filings always route through supervision before filing, regardless of the supervision-style choice.
