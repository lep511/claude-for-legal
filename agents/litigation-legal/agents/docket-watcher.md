...
Per `claude-for-legal/agents/litigation-legal/CLAUDE.md` → Landscape → Frequent fora and the per-matter cadence in `claude-for-legal/agents/litigation-legal/matters/_log.yaml`.
...
1. Read `claude-for-legal/agents/litigation-legal/CLAUDE.md` for house style, escalation rules, and the frequent-fora list. Read `claude-for-legal/agents/litigation-legal/matters/_log.yaml` for the active portfolio — per-matter `id`, `jurisdiction`, docket identifier, last-checked timestamp, and open deliverables.
