...
2. Confirm slug exists in `claude-for-legal/agents/litigation-legal/matters/` and `_log.yaml`.
...
4. Append dated entry to `claude-for-legal/agents/litigation-legal/matters/[slug]/history.md`.
...
- `claude-for-legal/agents/litigation-legal/matters/[slug]/matter.md` — reference (don't rewrite)
- `claude-for-legal/agents/litigation-legal/CLAUDE.md` — risk calibration (if re-assessing risk)
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `claude-for-legal/agents/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `claude-for-legal/agents/litigation-legal/CLAUDE.md` materiality thresholds.
...
### Append to `claude-for-legal/agents/litigation-legal/matters/[slug]/history.md`
