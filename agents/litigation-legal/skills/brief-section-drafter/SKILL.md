...
1. Load `claude-for-legal/agents/litigation-legal/CLAUDE.md` → case theory, house style.
...
`claude-for-legal/agents/litigation-legal/CLAUDE.md` → case theory, house style (citation format, structure, tone, length norms).
...
Per `claude-for-legal/agents/litigation-legal/CLAUDE.md`:
...
**Before the brief is filed (the consequential act — this skill drafts, but the gate runs at the filing step regardless of who triggers it):** Read `## Who's using this` in `claude-for-legal/agents/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
