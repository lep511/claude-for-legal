...
1. Load `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load `claude-for-legal/agents/litigation-legal/CLAUDE.md` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `claude-for-legal/agents/litigation-legal/CLAUDE.md`; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/checklist.md` (post-send checklist).
...
- `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/intake.md` — required; refuse to proceed if missing
- `claude-for-legal/agents/litigation-legal/CLAUDE.md` → Demand-letter practice (seed-doc paths, insurance-tender timing, materiality threshold for matter creation), house style (privilege markings, outside counsel directive format for tone reference). **Tone, compliance period, marking, and signer come from `## Posture for this matter` — they are matter-level, not practice-level.**
- `claude-for-legal/agents/litigation-legal/matters/_log.yaml` — to check for existing related matters (same counterparty) and offer cross-link
...
Check `claude-for-legal/agents/litigation-legal/CLAUDE.md` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `claude-for-legal/agents/litigation-legal/CLAUDE.md` privilege conventions exactly.

## Output

### Primary: `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/draft-v[N].docx`
...
### `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/checklist.md` — the post-send checklist
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `claude-for-legal/agents/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `claude-for-legal/agents/litigation-legal/CLAUDE.md` medium-severity band
...
> Create a tracked matter in `claude-for-legal/agents/litigation-legal/matters/_log.yaml`? (default: [yes/no])
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `claude-for-legal/agents/litigation-legal/demand-letters/` only.
