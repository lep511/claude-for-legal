...
1. Load `claude-for-legal/agents/litigation-legal/CLAUDE.md` → demand-letter practice, landscape, risk calibration.
...
- Desired outcome dollar value ≥ the medium-severity band from `claude-for-legal/agents/litigation-legal/CLAUDE.md` risk calibration
- Counterparty is a customer, competitor, or frequent adversary per `claude-for-legal/agents/litigation-legal/CLAUDE.md` landscape
- User ran with `--full`
...
`[type]-[counterparty-short]-[yyyy-mm]`. Confirm uniqueness in `claude-for-legal/agents/litigation-legal/demand-letters/`.

### `claude-for-legal/agents/litigation-legal/demand-letters/[slug]/intake.md`
...
- Run the conflicts check. If the counterparty is a customer or known entity, flag that this should clear conflicts (per `claude-for-legal/agents/litigation-legal/CLAUDE.md`) before sending — but the check itself lives in the matter-intake workflow or outside this skill.
