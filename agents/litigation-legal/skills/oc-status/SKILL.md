...
5. Write markdown to `claude-for-legal/agents/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`.
6. If Gmail MCP authenticated: create Gmail drafts. Else: markdown-only, note in summary.
7. Write `claude-for-legal/agents/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md` — what ran, what was skipped and why.
...
- `claude-for-legal/agents/litigation-legal/matters/_log.yaml` — the filtering and field source
- `claude-for-legal/agents/litigation-legal/matters/[slug]/matter.md` — matter context (current posture, open questions)
- `claude-for-legal/agents/litigation-legal/matters/[slug]/history.md` — recent events to inform what to ask about
- `claude-for-legal/agents/litigation-legal/CLAUDE.md` → outside counsel directive style, signer name/email, budget posture
...
**Subject:** per house convention (from `claude-for-legal/agents/litigation-legal/CLAUDE.md` outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per `claude-for-legal/agents/litigation-legal/CLAUDE.md` budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From `claude-for-legal/agents/litigation-legal/CLAUDE.md` signer default for OC directives.]
```

Adapt tone per `claude-for-legal/agents/litigation-legal/CLAUDE.md` outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `claude-for-legal/agents/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`
...
**From:** [signer name / email from `claude-for-legal/agents/litigation-legal/CLAUDE.md`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (`claude-for-legal/agents/litigation-legal/CLAUDE.md` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
After processing all matters, write `claude-for-legal/agents/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md`:
...
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match `claude-for-legal/agents/litigation-legal/CLAUDE.md`.
