...
Output from the oc-status workflow. Per-run folders dated by day; each contains one markdown file per matter drafted, plus a `_summary.md`.
...
Weekly (Monday AM) when scheduled. Register the schedule with the oc-status workflow (--setup-schedule).

Ad-hoc any time with the oc-status workflow (default filter) or the oc-status workflow (--slug=[slug]) (one matter).
