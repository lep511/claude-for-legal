...
1. Load `claude-for-legal/agents/privacy-legal/CLAUDE.md` → DPA playbook. If placeholders, stop and prompt setup.
...
DPAs come in two flavors and the review is nearly opposite for each. When a customer sends their DPA, we're defending our operational flexibility. When we send one to a vendor, we're protecting our (and our customers') data. Both reviews read from the same `claude-for-legal/agents/privacy-legal/CLAUDE.md` playbook but from opposite rows.

## First: which direction?

Before anything else, establish:

- **We are the processor** → customer is sending us their DPA → read `claude-for-legal/agents/privacy-legal/CLAUDE.md` → "When we are the processor" table
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `claude-for-legal/agents/privacy-legal/CLAUDE.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `claude-for-legal/agents/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `claude-for-legal/agents/privacy-legal/CLAUDE.md` → `## DPA playbook`. Also read `## Privacy policy commitments` — the DPA can't contradict what the privacy policy promises.
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `claude-for-legal/agents/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
