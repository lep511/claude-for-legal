...
1. Load `claude-for-legal/agents/product-legal/CLAUDE.md` → framework + calibration. Stop if placeholders.
...
Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per `claude-for-legal/agents/product-legal/CLAUDE.md`), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.

## Load calibration

Read `claude-for-legal/agents/product-legal/CLAUDE.md`:
...
For each category in `claude-for-legal/agents/product-legal/CLAUDE.md` → Review framework. If the team doesn't have one, use the 8-category default below. The categories are stable framing concepts; within each category, research the regulatory regimes applicable to the product's sector, audience, and jurisdictions before calibrating severity. What blocks in one jurisdiction or sector may be routine in another — `claude-for-legal/agents/product-legal/CLAUDE.md` captures the team's calibration.
...
For each finding, check against the calibration table in claude-for-legal/agents/product-legal/CLAUDE.md:
...
Format per `claude-for-legal/agents/product-legal/CLAUDE.md` → Launch review process → output format. Prepend the work-product header from `claude-for-legal/agents/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in `claude-for-legal/agents/product-legal/CLAUDE.md`. If the Role is Non-lawyer:
>
...
**Output 1 — Privileged launch review memo.** The full analysis assembled in Step 5: work-product header, bottom line, findings by category with risk rationale, action items, escalations, notes for next time, citation check. This is internal legal work product. Keep it in your matter file (Drive, DMS, or wherever `claude-for-legal/agents/product-legal/CLAUDE.md` says review docs go). Distribute only to people inside the privilege circle.
...
- It doesn't retroactively calibrate. If this launch turns out fine (or badly) in a way that should update the calibration table, a human updates claude-for-legal/agents/product-legal/CLAUDE.md.
