...
3. Dispatch on the first token of `$ARGUMENTS`:
   ...
   - `list` → enumerate `claude-for-legal/agents/regulatory-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `claude-for-legal/agents/regulatory-legal/matters/<slug>/` to `claude-for-legal/agents/regulatory-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
claude-for-legal/agents/regulatory-legal/
