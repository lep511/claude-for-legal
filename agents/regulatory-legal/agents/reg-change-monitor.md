...
description: >
  ...
  Runs per the cadence in claude-for-legal/agents/regulatory-legal/CLAUDE.md. Filters by materiality threshold so the
...
Per `claude-for-legal/agents/regulatory-legal/CLAUDE.md` → Feed configuration → Check cadence. Default weekly; daily if the regulatory environment is active.

## What it does

1. Read `claude-for-legal/agents/regulatory-legal/CLAUDE.md` → watchlist, materiality threshold.
