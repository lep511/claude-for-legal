...
description: >
  ...
  within a rolling 12-month window (configurable in `claude-for-legal/agents/commercial-legal/CLAUDE.md`).
...
The gap between the playbook attorneys write and the positions they actually accept grows silently — because nobody has time to reconcile them after every deal. This agent watches the deviation log, detects when a position is being overridden consistently, and proposes a specific update to `claude-for-legal/agents/commercial-legal/CLAUDE.md`. The attorney approves or rejects. The playbook stays alive.
...
Both values are configurable in `claude-for-legal/agents/commercial-legal/CLAUDE.md` under `## Playbook monitor settings`:
...
If these fields are absent from `claude-for-legal/agents/commercial-legal/CLAUDE.md`, use the defaults above.

## What it does

### Step 1 — Read the practice profile and log

1. Read `claude-for-legal/agents/commercial-legal/CLAUDE.md` in full. Extract:
...
2. Read `claude-for-legal/agents/commercial-legal/deviation-log.yaml`. Filter out:
...
If no clause crosses the threshold: log the check to `claude-for-legal/agents/commercial-legal/playbook-monitor-log.yaml` and stop. Do not notify the attorney.
...
2. **Current playbook language** (exact text from `claude-for-legal/agents/commercial-legal/CLAUDE.md`)
...
Current language in `claude-for-legal/agents/commercial-legal/CLAUDE.md`:
...
Write all proposals to `claude-for-legal/agents/commercial-legal/playbook-proposals.md`. Overwrite any existing file — stale unreviewed proposals are replaced, not accumulated.
...
- run_at: [ISO datetime]
  ...
  proposals_file: claude-for-legal/agents/commercial-legal/playbook-proposals.md
```
...
[A] Accept — apply proposed language to `claude-for-legal/agents/commercial-legal/CLAUDE.md`
...
```
Updating `claude-for-legal/agents/commercial-legal/CLAUDE.md`:
...
5. **Reject / Defer:** log to `claude-for-legal/agents/commercial-legal/playbook-monitor-log.yaml` with reason if given. Do not modify `claude-for-legal/agents/commercial-legal/CLAUDE.md`. A rejected proposal is not re-raised until a new pattern emerges after the rejection date.

6. After all proposals resolved, show summary:

```
Review complete.
[N] accepted and applied to `claude-for-legal/agents/commercial-legal/CLAUDE.md`
[N] rejected
[N] deferred to next cycle
[N] edited and applied

`claude-for-legal/agents/commercial-legal/CLAUDE.md` last updated: [timestamp]
Next playbook check: after [N] more deals are logged
```

7. Archive: rename `claude-for-legal/agents/commercial-legal/playbook-proposals.md` to `claude-for-legal/agents/commercial-legal/playbook-proposals-[YYYYMMDD].md`. The active file is now clear.

## What this agent does NOT do

- Modify `claude-for-legal/agents/commercial-legal/CLAUDE.md` without explicit per-change attorney confirmation
