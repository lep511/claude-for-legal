...
description: >
  ...
  `claude-for-legal/agents/commercial-legal/CLAUDE.md`. Flags deviations, assesses risk, generates
  specific redline language, and routes to the right approver. Loaded by
  the review workflow when a vendor MSA, services agreement, or similar is detected.
...
Read a vendor agreement against the playbook this team actually uses (in `claude-for-legal/agents/commercial-legal/CLAUDE.md`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `claude-for-legal/agents/commercial-legal/CLAUDE.md`.** If it's missing or still has placeholders, surface this bounce:
...
>
> **Two choices:**
> - Configure this agent in Settings (2 minutes) to configure your profile, then I'll review tailored to YOUR playbook.
...
The playbook in `claude-for-legal/agents/commercial-legal/CLAUDE.md` is the source of truth. It tells you:
...
Check the "one thing" from `claude-for-legal/agents/commercial-legal/CLAUDE.md` first. If present:
...
For each playbook category in `claude-for-legal/agents/commercial-legal/CLAUDE.md`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `claude-for-legal/agents/commercial-legal/CLAUDE.md`]
...
**If they won't move:** [the fallback from `claude-for-legal/agents/commercial-legal/CLAUDE.md`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `claude-for-legal/agents/commercial-legal/CLAUDE.md`, or a deal-breaker. |
...
Severity is always applied *against `claude-for-legal/agents/commercial-legal/CLAUDE.md`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `claude-for-legal/agents/commercial-legal/CLAUDE.md`.
...
Check the escalation matrix in `claude-for-legal/agents/commercial-legal/CLAUDE.md` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `claude-for-legal/agents/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `claude-for-legal/agents/commercial-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `claude-for-legal/agents/commercial-legal/CLAUDE.md` house-style section.
...
- [ ] `claude-for-legal/agents/commercial-legal/CLAUDE.md` was loaded and quoted — not generic market positions
