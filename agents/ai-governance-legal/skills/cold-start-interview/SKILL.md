...
description: >
  ...
  `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` from
...
1. Check `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` — if populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `claude-for-legal/plugins/cache/claude-for-legal/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and tell the user what was migrated.
6. Write `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
Read `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`:
...
If a CLAUDE.md exists at the old cache path `claude-for-legal/plugins/cache/claude-for-legal/ai-governance-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
> "This plugin maintains: your practice profile (governance tiers, red lines, policy commitments), a use-case registry, impact assessments, and vendor AI reviews — all in `claude-for-legal/agents/ai-governance-legal/`. It learns how you actually work — your practice, your risk calibration, your house conventions — and writes that into a plain-text file the plugin reads from every time. Everything you answer can be changed later."
