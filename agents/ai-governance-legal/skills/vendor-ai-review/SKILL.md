...
1. Read `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`.
...
Read `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance`. Also read `## AI policy commitments`
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
If `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` doesn't define a position for a term on this list, ask: "Your playbook doesn't cover [term]. What's your default position, your acceptable fallback, and your automatic no? I'll add it to `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` so the next review is consistent."

---

## Playbook comparison

For each term above, compare what we found to the positions in `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`.
...
> **Our position:** [from `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`.
...
Cross-check the vendor's terms against our AI policy commitments in `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `claude-for-legal/agents/ai-governance-legal/CLAUDE.md`, or "escalate — outside fallback"
