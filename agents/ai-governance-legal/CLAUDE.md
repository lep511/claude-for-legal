...
`[PLACEHOLDER]`, configure this agent in Settings.*
...
**Inventory file:** `claude-for-legal/agents/ai-governance-legal/ai-systems.yaml`
...
tagged `[verify]`, and routed to the aia-generation workflow for
...
Manage the inventory with the ai-inventory workflow —
...
When the user asks a question in this plugin's practice area — not just when they invoke a skill — read the practice profile at `claude-for-legal/agents/ai-governance-legal/CLAUDE.md` (and `claude-for-legal/agents/company-profile.md`) first, and apply it. If it's populated, answer as the configured assistant:
...
*Only relevant for multi-client practices (private practice — solo, small firm, large firm). If you're in-house AI governance counsel for one company, this section is off and nothing below applies — skills use practice-level context automatically, and the matter-workspace workflow is not something you need.*
