...
Daily during active diligence. Checklist status per `claude-for-legal/agents/corporate-legal/CLAUDE.md` → Deal team briefing cadence.

## Integrations

Posting to Slack requires a Slack MCP server in your environment. This plugin does not bundle one. If no Slack MCP is configured, write the VDR update and checklist status to a file in `claude-for-legal/agents/corporate-legal/deals/[code]/updates/[date].md` and notify the user — do not fail silently.

VDR tools (Box, Intralinks, Datasite) are likewise external MCPs — if none are connected, prompt the user for the VDR export or ask them to update `claude-for-legal/agents/corporate-legal/deals/[code]/vdr-inventory.md` manually.
