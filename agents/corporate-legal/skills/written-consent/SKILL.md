...
1. Load `claude-for-legal/agents/corporate-legal/CLAUDE.md` → Board & Secretary (consents repository, resolution language, state of incorporation, board composition).
...
5. Search consents repository for closest precedent. If no repository: use seed consents from `claude-for-legal/agents/corporate-legal/CLAUDE.md`.
...
- `claude-for-legal/agents/corporate-legal/CLAUDE.md` → `## Board & Secretary`:
...
- **Signatories:** Full board, or a specific committee? If the `claude-for-legal/agents/corporate-legal/CLAUDE.md` written-consent scope says certain actions require a meeting rather than consent, flag it now.
...
Extract the format from the seed consents in `claude-for-legal/agents/corporate-legal/CLAUDE.md`. Note that no precedent search is available — the draft will follow house format but without substantive precedent matching. Flag this to the user:
...
Check the state of incorporation in `claude-for-legal/agents/corporate-legal/CLAUDE.md`. Research the written-consent requirements for that state before drafting:
...
If `claude-for-legal/agents/corporate-legal/CLAUDE.md` records a house position on any of these questions, apply it and note the legal backstop being relied on. Add a short "State-law notice" block to the output summarizing what you confirmed (or flagged) so the user isn't left wondering.

---

## Step 4.5: Consequential-action gate (execute consent)

**Before proceeding to output:** Read `## Who's using this` in `claude-for-legal/agents/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
...
□ [Director Name 2]
□ [Director Name 3]
[etc. — pulled from board composition in `claude-for-legal/agents/corporate-legal/CLAUDE.md`]
