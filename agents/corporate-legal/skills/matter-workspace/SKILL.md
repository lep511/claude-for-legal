...
**Default state is off.** In-house users never see this — they run at practice-level only. Matter workspaces turn on at cold-start for private-practice users, or by editing `## Matter workspaces` in the practice-level CLAUDE.md. If `Enabled` is `✗`, this skill does not run; the matter-workspace workflow explains the disabled state and suggests the Settings page to reconfigure for users who actually need matter isolation.
...
```
claude-for-legal/agents/corporate-legal/
