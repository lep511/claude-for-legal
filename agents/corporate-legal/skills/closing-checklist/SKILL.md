...
1. Read `claude-for-legal/agents/corporate-legal/deals/[code]/closing-checklist.yaml` and use the modes below.
...
Lives at `claude-for-legal/agents/corporate-legal/deals/[code]/closing-checklist.yaml`. Structure:
...
```
Use the closing-checklist workflow
...
**Before producing a "ready to close / all CPs satisfied" certification or closing memo:** Read `## Who's using this` in `claude-for-legal/agents/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
